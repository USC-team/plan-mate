...
class StatesRepositoryImpTest {
    ...
    fun `getAllStates should return mapped domain States when data source returns DTOs`() {
        runTest {
            ...
            coEvery { mockStatesDataSource.getAllStates(projectId.toString()) } returns listOf(dto1, dto2)
    ...
    fun `getAllStates should wrap exception when data source fails`() {
        runTest {
            // Given
            val projectId = random()
            coEvery { mockStatesDataSource.getAllStates(projectId.toString()) } throws RuntimeException("I/O error")
    ...
    fun `createState should wrap exception when data source fails`() {
        runTest {
            ...
            coEvery { mockStatesDataSource.createState(any()) } throws RuntimeException("Disk full")
    ...
    fun `updateState should wrap exception when data source fails`() {
        runTest {
            ...
            coEvery { mockStatesDataSource.updateState(any()) } throws RuntimeException("Error")
    ...
    fun `deleteState should wrap exception when data source fails`() {
        runTest {
            // Given
            val stateId = random()
            coEvery { mockStatesDataSource.deleteState(stateId.toString()) } throws RuntimeException("Delete failure")
