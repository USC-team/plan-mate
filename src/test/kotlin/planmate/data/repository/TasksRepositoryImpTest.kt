...
class TasksRepositoryImpTest {
    ...
    fun `getAllTasks should returns mapped domain tasks when data source provides DTOs`() {
        runTest {
            ...
            coEvery { mockTasksDataSource.getAllTasks(projectId.toString()) } returns listOf(dto1, dto2)
    ...
    fun `getAllTasks should wraps exception when data source fails`() {
        runTest {
            // Given
            val projectId = Uuid.random()
            coEvery { mockTasksDataSource.getAllTasks(projectId.toString()) } throws Exception("Error")
    ...
    fun `createTask should delegates to data source when no existing ID`() {
        runTest {
            ...
            coEvery { mockTasksDataSource.getAllTasks(projectId.toString()) } returns emptyList()
    ...
    fun `createTask should throws when ID already exists`() {
        runTest {
            ...
            coEvery { mockTasksDataSource.getAllTasks(projectId.toString()) } returns listOf(existingDto)
    ...
    fun `createTask should wraps exception from data source`() {
        runTest {
            ...
            coEvery { mockTasksDataSource.getAllTasks(projectId.toString()) } returns emptyList()
            coEvery { mockTasksDataSource.createTask(any()) } throws Exception("Error")
    ...
    fun `updateTask should wraps exception from data source`() {
        runTest {
            ...
            coEvery { mockTasksDataSource.getAllTasks(projectId.toString()) } returns listOf(task.fromDomain(task))
            coEvery { mockTasksDataSource.updateTask(any()) } throws Exception("error")
    ...
    fun `deleteTask should wraps exception from data source`() {
        runTest {
            // Given
            val taskId = Uuid.random()
            coEvery { mockTasksDataSource.deleteTask(taskId.toString()) } throws Exception("Delete failure")
